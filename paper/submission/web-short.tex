...
\acmDOI{}
\acmISBN{}
\acmConference[WWW 2027 submission]{The Web Conference 2027 Short Papers submission}{May 10--14, 2027}{Dublin, Ireland}
\acmYear{2027}
...
As agents mediate transactions on the Web, platforms must distinguish evidence about user value from successful task execution. This is a measurement problem for agentic Web users and platform product decisions. We ask: \emph{does a declared log support a declared product comparison under an explicit calibration model?} Our executable audit exposes compatible decisions and assumptions, using actual model responses to synthetic objectives, controlled channels, and an evidence interface. It does not estimate actual demand.

Earlier studies examine delegated choice \cite{kops2026}, preference transmission \cite{kraft2026}, and alignment through revealed preferences \cite{suleymanov2026}. They ask how agent behavior relates to the preferences of its principal. Rich variation across menus can identify quantities that coarse transaction categories cannot. Controlled agent-choice experiments also have direct precedents \cite{cherep2025}. Our operational focus is a particular future product contrast and the uncertainty left by a specified logging scheme. The statistical foundations are established experiment comparison, label-shift estimation, and partial identification with measurement error \cite{blackwell1953,lipton2018,finkelstein2021}.
...
\begin{equation}
\begin{aligned}
\mathcal P_q&=\{p\geq0:\mathbf1^Tp=1,Ap=q\},\quad \Delta=d^Tp,\\
[L,U]&=\left[\min_{p\in\mathcal P_q}d^Tp,\ \max_{p\in\mathcal P_q}d^Tp\right].
\end{aligned}
\end{equation}
...
We account for uncertainty in the calibrated channel and observed frequencies using a linear program in joint masses. Write $J_{ak}=A_{ak}p_k$ and impose
\[
\begin{aligned}
